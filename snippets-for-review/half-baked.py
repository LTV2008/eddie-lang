...
class Interpreter:
    ...
    def __init__(self):
        ...
        self._scope_stack: List[Dict[str, Any]] = []   # lexical scopes
    ...
    def _execute_if_block(self, lines: List[str], start_idx: int) -> int:
        ...
        while i < len(lines):
            ...
            if line.startswith("if") or line.startswith("else if"):
                cond_match = re.match(r"(if|else if)\s*\((.*)\)\s*{", line)
                if not cond_match:
                    raise InterpreterError("Malformed if/else if header", i + 1)
                _, cond_expr = cond_match.groups()
                condition = self._evaluate_expression(cond_expr, i + 1)
